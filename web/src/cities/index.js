...
const getStationsData = async city => {
  const res = await fetch(
    `https://api.jcdecaux.com/vls/v1/stations?contract=${city}&apiKey=${process.env.REACT_APP_BIKES}`
  )
...
export const getCities = async () => {
  const res = await fetch(
    `https://api.jcdecaux.com/vls/v1/contracts?apiKey=${process.env.REACT_APP_BIKES}`
  )
  const data = await res.json()
